Module Mesh
    ...
    End interface
    Contains
      Subroutine InitialGrid(SPoint,EPoint,ReS,ReE,NI,NJ,NK,Irec,Jrec,Krec,    &
                                                                   TGrid,Lref)
        Implicit none
        ...
        dl = EPoint%y-ReE%y
        call NewtonRaphson(beta,dl,dy,IJKsize)
        y(1) = SPoint%y
        do j = 2,2+IJKsize
        ...
        dl = EPoint%z-ReE%z
        call NewtonRaphson(beta,dl,dz,IJKsize)
        z(1) = SPoint%z
        do k = 2,2+IJKsize
        ...
        dl = ReS%x-SPoint%x
        IJKsize = (NI-Irec)/2
        call NewtonRaphson(beta,dl,dx,IJKsize)
        x(1) = SPoint%x
        do i = 2,1+IJKsize
      ...
      Subroutine InitialUVGrid(PGrid,TGrid,UVW,Lref)
        Implicit none
