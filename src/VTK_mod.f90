...
module vtk
  ...
  subroutine VTK_open_file(prefix, dir, proc_rank, num_procs, restart, itera, fd)
    IMPLICIT NONE
      ...
      if ( present(proc_rank) ) then
        ...
        if(present(itera)) then
          f=trim(adjustl(dir))//'Paraview/'//trim(fd%prefix)//                &
            trim(adjustl(rank))//"_"//trim(adjustl(snapshot))//".vtk"
        end if
  ...
  subroutine VTK_write_mesh_2d(fd, x, y)
    ...
    character(len=:),allocatable    :: codex,codey,codez
    INTEGER, PARAMETER :: nk=1
    
    z(1)=0.d0
    codex=encode_binary_dataarray(x=x)
    codey=encode_binary_dataarray(x=y)
    codez=encode_binary_dataarray(x=z) 
    ...
    write(unit=fd%unit) trim(GRID_header),codex,newline
    write(buf2,'(i8)') fd%nj
 !   print*,'+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++'
    GRID_header="Y_COORDINATES "//trim(adjustl(buf2))//" float"//newline
    write(unit=fd%unit) trim(GRID_header),codey,newline
 !   print*,'-------------------------------------------------------------------'
    write(buf2,'(i8)') nk
    GRID_header="Z_COORDINATES "//trim(adjustl(buf2))//" float"//newline
    write(unit=fd%unit) trim(GRID_header),codez,newline
  ...
  subroutine VTK_write_scalar_2d(fd, name, field)
    ...
    character(len=:),allocatable :: codef
    character(len=256) :: VAR_header
    
    codef=encode_binary_dataarray(x=field)
    ...
    write(unit=fd%unit) trim(VAR_header),codef,newline
