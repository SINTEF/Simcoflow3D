...
Program Main
    ...
    Open(unit=5,file='/home/elena-roxanap/Documents/Iceload/simco3d/src/Test/Vessel/input.dat',action='read')
    Read(5,*)
    ...
    STLfilename=trim('/home/elena-roxanap/Documents/Iceload/simco3d/src/Test/Vessel/Akademik_sergey_Vavilov.stl')
    !STLfilename=trim('/home/elena-roxanap/Documents/Iceload/simco3d/src/Test/Vessel/Ship.stl')
    ...
    SPoint%x = -180!-200.d0/2.
    SPoint%y = -36!-160.d0/2.
    SPoint%z = -40!-500.d0/2.
    EPoint%x = 180!200.d0/2.
    EPoint%y = 36!120.d0/2.
    EPoint%z = 40!500.d0/2.
    ReS%x = -120.d0!-1.d0
    ReS%y = -18.d0!-1.d0
    ReS%z = -20.d0!-1.d0
    ReE%x = 120.d0!1.d0
    ...
    !call InitialGridVessel(Lref,SPoint,EPoint, Pgrid)
    ...
    print*, 'out of read stl'
    print*, '--------------The dimensions of the ship are in [mm] and should be transformed in [m]--------------'
    call ComSTL%ModifySTLFile(1000.d0, CentPoint, 1.d0)

    minp1= minval(COmSTL%tri(:)%ptr(3)%p(1))
    maxp1= maxval(COmSTL%tri(:)%ptr(3)%p(1))
    print*, 'minp1,maxp1',minp1,maxp1
    minp1= minval(COmSTL%tri(:)%ptr(2)%p(1))
    maxp1= maxval(COmSTL%tri(:)%ptr(2)%p(1))
    print*, 'minp1,maxp1',minp1,maxp1
    minp1= minval(COmSTL%tri(:)%ptr(1)%p(1))
    maxp1= maxval(COmSTL%tri(:)%ptr(1)%p(1))
    print*, 'minp1,maxp1',minp1,maxp1
    minp1= minval(COmSTL%tri(:)%ptr(3)%p(2))
    maxp1= maxval(COmSTL%tri(:)%ptr(3)%p(2))
    print*, 'minp1,maxp1',minp1,maxp1
    minp1= minval(COmSTL%tri(:)%ptr(2)%p(2))
    maxp1= maxval(COmSTL%tri(:)%ptr(2)%p(2))
    print*, 'minp1,maxp1',minp1,maxp1
    minp1= minval(COmSTL%tri(:)%ptr(1)%p(2))
    maxp1= maxval(COmSTL%tri(:)%ptr(1)%p(2))
    print*, 'minp1,maxp1',minp1,maxp1
    minp1= minval(COmSTL%tri(:)%ptr(3)%p(3))
    maxp1= maxval(COmSTL%tri(:)%ptr(3)%p(3))
    print*, 'minp1,maxp1',minp1,maxp1
    minp1= minval(COmSTL%tri(:)%ptr(2)%p(3))
    maxp1= maxval(COmSTL%tri(:)%ptr(2)%p(3))
    print*, 'minp1,maxp1',minp1,maxp1
    minp1= minval(COmSTL%tri(:)%ptr(1)%p(3))
    maxp1= maxval(COmSTL%tri(:)%ptr(1)%p(3))
    print*, 'minp1,maxp1',minp1,maxp1
    !scalefactor=0.5!(maxp1-minp1)/lRef
    ...
    call PrintResultVTK(PGrid,Var,PCell,itt)
