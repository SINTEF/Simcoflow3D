Module Clsvof
   ...
   ! Date : 20.09.2019
    ...
    public:: InitialClsvofFluidField,InitialClsvofLiquidField,Clsvof_Scheme,   &
             ...
             BoundaryConditionLvsVof, BoundaryConditionLvsVofFluid,InitialiseClsvof, LvsObject
    ...
    !
    subroutine LvsObject(TCell,ComSTL,Tgrid)
      !
      ...
      type(simcoSTL),                  intent(in)     :: ComSTL
      type(Grid),                      intent(in)     :: TGrid
      integer(kind=it4b)                              :: i,j,k,ii,jj,kk,i1,j1,k1,bw
      integer(kind=it4b), dimension(:,:),allocatable  :: GridIndex
      integer(kind=it4b), dimension(:,:), allocatable :: BoxSizePos, BoxSizeNeg
      ...
      do i=1,ComSTL%ntri
        ...
        BoxSizePos(i,1) = max(4,int(MaxObjX/TGrid%dx(Imax/2,1,1))+4)
        BoxSizePos(i,2) = max(4,int(MaxObjY/TGrid%dy(1,Jmax/2,1))+4)
        BoxSizePos(i,3) = max(4,int(MaxObjZ/TGrid%dz(1,1,Kmax/2))+4)

        BoxSizeNeg(i,1) = min(-4,int(MinObjX/TGrid%dx(Imax/2,1,1))-4)
        BoxSizeNeg(i,2) = min(-4,int(MinObjY/TGrid%dy(1,Jmax/2,1))-4)
          ...
          do jj=BoxSizeNeg(i,2),BoxSizePos(i,2)
            do kk=BoxSizeNeg(i,3),BoxSizePos(i,3)
              ...
              if(dabs(tempd)<dabs(TCell%phi(i1,j1,k1))) then
                ...
                TCell%nx(i1,j1,k1)  = npoint%v(1)
                TCell%ny(i1,j1,k1)  = npoint%v(2)
                TCell%nz(i1,j1,k1)  = npoint%v(3)
                TCell%vof(i1,j1,k1) = 1.d0
              end if
            end do
          end do
        end do
      ...
      do i = 1, Imax
        do j = 1, Jmax
          do k = 1, Kmax
            if(dabs(TCell%phi(i,j,k))<0.9d0*maxLvs) then
              pt%p(1) = TGrid%x(i,1,1)
              pt%p(2) = TGrid%y(1,j,1)
              pt%p(3) = TGrid%z(1,1,k)
              Isin = RayCasting(pt,ComSTL)
              if(Isin.eqv..true.) then
                TCell%phi(i,j,k) = -dabs(TCell%phi(i,j,k))
              else
                TCell%phi(i,j,k) = dabs(TCell%phi(i,j,k))
              end if
            end if
      ...
      do i=1,Imax
        do j=1,Jmax
          do k=1,Kmax
            ...
            ny = TCell%ny(i,j,k)
            nz = TCell%nz(i,j,k)
            s=TCell%phi(i,j,k)+0.5*(dabs(nx)*TGrid%dx(i,j,k)+               &
                                    dabs(ny)*TGrid%dy(i,j,k)+               &
                                    dabs(nz)*TGrid%dz(i,j,k))
    ...
    !
    pure subroutine ComputeGridIndexObject(TGrid,ComSTL,GridIndex)
      !
      !! Determine the cell where the element triangle of STL locates at
      type(Grid),intent(in)                                       :: TGrid
      type(simcoSTL),intent(in)                                   :: ComSTL
      integer(kind=it4b),dimension(:,:),allocatable,intent(inout) :: GridIndex
      ...
      do j=1,ComSTL%ntri
        ...
        do i=1,Imax-1
          if(TGrid%x(i,1,1)<=TriCenter%p(1).and.TGrid%x(i+1,1,1)>TriCenter%p(1)) then
            GridIndex(j,1)=i
          end if
        end do
        do i=1,Jmax-1
          if(TGrid%y(1,i,1)<=TriCenter%p(2).and.TGrid%y(1,i+1,1)>TriCenter%p(2)) then
            GridIndex(j,2)=i
          end if
        end do
        do i=1,Kmax-1
          if(TGrid%z(1,1,i)<=TriCenter%p(3).and.TGrid%z(1,1,i+1)>TriCenter%p(3)) then
            GridIndex(j,3)=i
          end if
        end do
      end do
